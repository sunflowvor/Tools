...
number_count = 50
number_count_all = number_count * 2 + 1
...
for i in range(len(lidar_lists)):
	...
	start_timestamp = float(key_timestamp) - number_count * 0.1
	#print(start_timestamp)
	#assert 0
	sequences_pose_list = [] 
	timestamp = start_timestamp
	for i in range(number_count_all):
		...
		if key_pose is None:
			print(timestamp)
			#assert 0 
	assert len(sequences_pose_list) == number_count_all
	#assert 0 
...
for i in range(len(lidar_lists)):
	...
	for j in range(number_count_all - 1):
		if j == int(np.floor(number_count_all/2)):
			continue
		target_index = str(i).zfill(6) + "_" + str(j - number_count) + ".pcd"
		target_point = o3d.io.read_point_cloud(os.path.join(sequence_lidar_path, str(i), target_index))
		key_pose = sequences_pose[int(np.floor(number_count_all/2))]
		...
		if key_point_list is None:
			key_point_list = key_point
	...
	vis = o3d.visualization.Visualizer()
	vis.create_window()	
	vis.add_geometry(key_point_list)
	for n in range(len(clouds_list)):
		vis.add_geometry(clouds_list[n])
	vis.run()
